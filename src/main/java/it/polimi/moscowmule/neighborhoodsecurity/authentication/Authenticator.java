...
public class Authenticator {
	 ...
	 * @throws NoUserFoundException if the authToken is of no user
	 */
	public static int getUserId(String authToken) throws AuthorizationDBException, NoUserFoundException {
		...
		try {
			...
			PreparedStatement getStmt = connection.prepareStatement(
					"SELECT ID FROM gsx95369n3oh2zo6.authorization WHERE TOKEN = ? AND ISVALID = 1", ResultSet.TYPE_SCROLL_INSENSITIVE,
	 ...
	 * @return the status
	 * @throws NoUserFoundException if the id is of no user
	 * @throws AuthorizationDBException
	 */
	 ...
	 * @throws NoUserFoundException if no match
	...
	public static String generateToken(int id) throws AuthorizationDBException{
		...
		try {
			connection = Database.getConnection();
			PreparedStatement updStmt = connection.prepareStatement("UPDATE gsx95369n3oh2zo6.authorization SET TOKEN = ?, ISVALID = 1 WHERE ID = ?");
	...
	public static boolean invalidateToken(int id) throws AuthorizationDBException{
		...
		try {
			connection = Database.getConnection();
			PreparedStatement updStmt = connection.prepareStatement("UPDATE gsx95369n3oh2zo6.authorization SET ISVALID = 0 WHERE ID = ?");
			updStmt.setInt(1, id);
			updStmt.executeUpdate();
